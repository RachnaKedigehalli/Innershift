...
function ViewPatient() {
  ...
  const RescheduleDialog = (moduleAssignedId) => {
    ...
    const completeReschedule = () => {
      ...
      console.log(dict);
      axios
        .post("http://localhost:8080/api/v1/app/updateOrder", dict, auth)
        .then((response) => {
          onClose();
        });
    };
  ...
  const toggleLock = (moduleAssignedId, index) => {
    ...
    console.log(dict);
    axios
      .post("http://localhost:8080/api/v1/app/setLockedForModule", dict, auth)
      .then((response) => {
      ...
      <Card bg={DESKTOP_BG_MEDIUM}>
        <CardBody bg={DESKTOP_BG_MEDIUM}>
          <VStack spacing={3} align="left">
            <Heading flex={1}>
              {" "}
              <Text noOfLines={1} color="#285e61">
                {" "}
                {name}
              </Text>{" "}
            </Heading>
            <Text h={75} color="teal.700" noOfLines={3}>
              {" "}
              {desc}{" "}
            </Text>
            <Flex>
              <Text ml={1} color="teal.700">
                {" "}
                To be unlocked on:{" "}
              </Text>
              <Text ml={1} color="teal.700">
            ...
            <FormControl mt display="flex" alignItems="center">
              <FormLabel htmlFor="lock-unlock" mb="0" color="teal.700">
                Lock/Unlock:
              </FormLabel>
              <Switch
                defaultChecked={allModules[index].locked}
                onChange={() => toggleLock(moduleAssignedId, index)}
                id="lock-unlock"
              />
            </FormControl>
          </VStack>
  ...
  const fileExport = () => {
    const url =
      "http://localhost:8080/api/v1/app/exportPdf/" + location.state.id;
    window.open(url);
  };
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8080/api/v1/app/getMoodsByPid", details, auth)
      .then((response) => {
    ...
    axios
      .post("http://localhost:8080/api/v1/app/getModulesByPid", details, auth)
      .then((response) => {
        ...
        for (let i = 0; i < val.length; i++) {
          ...
          array.push({
            ...temp,
            moduleAssignedId: val[i].moduleAssignment.moduleAssignedId,
            scheduled: val[i].moduleAssignment.scheduled,
            locked: val[i].moduleAssignment.locked,
          });
        }
        setAllModules(array);
      });
